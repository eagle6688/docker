...
docker run \
  ...
  -p 2380:2380 \
  -v /var/data/etcd:/data \
  --name etcd-XIAOXIN310 \
  eagle6688/etcd \
  --name etcd-XIAOXIN310 \
  --data-dir /data \
  --listen-peer-urls http://0.0.0.0:2380 \
  --listen-client-urls http://0.0.0.0:2379 \
  --initial-advertise-peer-urls http://10.100.97.202:2380 \
  --initial-cluster etcd-master=http://10.100.97.46:2380,etcd-G510=http://10.100.97.64:2380,etcd-XIAOXIN310=http://10.100.97.202:2380 \
  --initial-cluster-state existing \
  --initial-cluster-token eagle-cluster \
  --advertise-client-urls http://10.100.97.202:2379